...
class LogginViewController: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        DispatchQueue.main.asyncAfter(deadline: .now()+4, qos: .default, flags: .assignCurrentContext) { [self] in
    ...
    @IBAction func logginButton(_ sender: UIButton) {
        if myTextField1.text == "" || myTextField2.text == "" {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now()+2, qos: .default, flags: .assignCurrentContext) {
                let movieVC = WhoWatchingViewController()
